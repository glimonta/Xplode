...
class TupleType : public TypeDeclaration {
  ...
  public:
    ...
    std::pair<TypeDeclaration*, int> *findAttribute(std::string att) {
      std::list< std::string>::iterator namesit;
      std::list< std::pair<TypeDeclaration*, int>* >::iterator typesit;

      for (namesit = names->begin(), typesit = types->begin();
           namesit != names->end() && typesit != types->end();
           ++namesit, ++typesit) {
        if (att == (*namesit))
          return (*typesit);
      }
